...
const page = ({ params }) => {
  ...
  if (session && session.user.role === "customer" && session.user.storeid === store_id) {
  ...
  if (status === "authenticated" && session.user.role === "customer" && session.user.storeid === store_id) {
    router.push(`/Customer/${store_id}`);
  ...
  return (
    <div className="flex justify-center bg-black w-full">
      <div className=" px-4 sm:px-10 border my-32 flex flex-col gap-7 py-14 justify-center items-center  bg-gray-800 border-white rounded-xl">
        <h1 className="text-4xl font-bold text-white">{store_id}</h1>
        <p className="text-gray-400 font-bold">Login as a Customer!</p>
